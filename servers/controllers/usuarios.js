...
function creaUsuario(req, res) {
    ...
    let validator = 'SELECT email, username FROM delilahrestoemilio.usuarios'
    ...
    connection.query(validator, function (err, info) {
        ...
        if (resultEmail || resultUsername) {
            ...
            res.status(409).json({ message: 'Este usuario ya está en uso, por favor modifique su nombre de usuario' })
            return
        }

        let sql = `INSERT INTO delilahrestoemilio.usuarios(username, fullname, email, phone, address, password, is_admin)
        VALUES ('${user.username}', '${user.fullname}', '${user.email}', '${user.phone}', '${user.address}', '${token}', 'false');`;

        connection.query(sql, function (err, user) {
            if (err) {
                console.log(err)
                res.status(500).json({ error: 'Debe ingresar todos los datos solicitados' });
...
function logIn(req, res) {
    ...
    let user = req.body
    let token = jwt.sign(user.password, firma);
    let sql = `SELECT password FROM delilahrestoemilio.usuarios WHERE usuarios.password = '${token}' AND usuarios.username = '${user.username}'`
   
    connection.query(sql, function (err, passwords) {

        if (err || passwords.length == 0) {
            res.status(500).json({message: "El usuario y/o el password no corresponden"})
            return
        } else {
...
function retornaUsuario(req, res) {
    ...
    if (userRol == 1) {
        ...
        sql = 'SELECT * FROM delilahrestoemilio.usuarios'
    } else {
        console.log(`Cuenta de Usuario`)
        sql = `SELECT * FROM delilahrestoemilio.usuarios WHERE id = ${userId}`
    }
