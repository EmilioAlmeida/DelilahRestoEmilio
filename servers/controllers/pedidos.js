...
function getAllOrders(req, res) {
    ...
    connection.query(sql, (err, orders) => {
        if (err) {
            console.log(err)
            res.status(500).json({ message: 'Error interno de Rol/pedidos' })
        } else {
            res.send(orders)
        }
    })
...
function newOrder(req, res) {
    ...
    let sql = `INSERT INTO delilahrestoemilio.pedidos(user_id, status, payment_method, update_time)
    ...
    connection.query(sql, (err, orders) => {
        if (err) {
            ...
            order.products.forEach(element => {
                ...
                let orderUnity = `INSERT INTO delilahrestoemilio.info_pedidos(order_id, product_id, quantity)
...
function updateOrder(req, res) {
    ...
    if (userRol == 1) {
        ...
        sql = `UPDATE delilahrestoemilio.pedidos
        SET status = '${update.status}'
        WHERE id = ${orderId}`
    } else {
        res.status(403).json({ message: "Solo un administrador puede modificar el estado del pedido" })
        return
    }
    ...
    connection.query(sql, (err, order) => {
        if (err) {
            console.log(err)
            res.status(500).json({ message: 'Error interno modificar pedidos' })
        } else {
            res.status(200).json({ message: 'Estado del pedido modificado', order })
        }
    })
...
function deleteOrder(req, res) {
    ...
    if (userRol == 1) {
        ...
        res.status(403).json({ message: "Solo un administrador puede eliminar un pedido" })
        return
    }
